...
const CustomDataTable = ({
  ...
  const handMenuItem = async (type, item) => {
    ...
    if (type === 'DELETE') {
      const res = await api.xoaDiemKhoanTrac(item.gid, {})
    }
    handleClose()
    fetchData()
  }
    ...
    <TableContainer sx={{ boxShadow: 'none' }}>
      <Table>
        <MDBox component="thead">
          <TableRow>
            <MyHeadCell align="center">Tên </MyHeadCell>
            <MyHeadCell align="center">Nội dung</MyHeadCell>
            <MyHeadCell align="center">Phân loại</MyHeadCell>
            <MyHeadCell align="center">Kinh độ</MyHeadCell>
            <MyHeadCell align="center">Vĩ độ</MyHeadCell>
          </TableRow>
        </MDBox>
        <TableBody>
          {dataRows?.map((item, idx) => (
            <TableRow key={idx}>
              <MybodyCell align="center">{item.name}</MybodyCell>
              <MybodyCell align="center">{item.popupinfo}</MybodyCell>
              <MybodyCell align="center">{item.pl}</MybodyCell>
              <MybodyCell align="center">{item.long}</MybodyCell>
              <MybodyCell align="center">{item.lat}</MybodyCell>

              <MybodyCell align={'center'}>
                <MDTypography
                  color="text"
                  onClick={(e) => handleClick(item.gid, e)}
                >
                  <Icon>more_vert</Icon>
                </MDTypography>
                <Menu
                  anchorEl={anchorEl && anchorEl[item.gid]}
                  keepMounted
                  open={Boolean(anchorEl && anchorEl[item.gid])}
                  onClose={handleClose}
                  anchorOrigin={{
                    vertical: 'bottom',
                    horizontal: 'center',
...
const DialogUpdateBlog = ({
  ...
  const them = async () => {
    const res = await api.themDiemKhoanTrac({
      name: activeItem?.name,
      popupinfo: activeItem?.popupinfo,
      pl: activeItem?.pl,
      long: parseFloat(activeItem?.long),
      lat: parseFloat(activeItem?.lat),
    })
    handleClose()
    setActiveItem({})
    fetchData()
  ...
  const sua = async () => {
    const res = await api.suaDiemKhoanTrac(activeItem?.gid, {
      name: activeItem?.name,
      popupinfo: activeItem?.popupinfo,
      pl: activeItem?.pl,
      long: parseFloat(activeItem?.long),
      ...
      <DialogContent dividers>
        ...
        <MDBox sx={{ width: '100%', mt: 2 }}>
          <TextField
            ...
            maxRows={10}
            rows={5}
            value={activeItem?.popupinfo}
            onChange={(e) =>
              setActiveItem({ ...activeItem, popupinfo: e.target.value })
            }
          />
        </MDBox>
        ...
        <MDBox sx={{ width: '100%', mt: 2 }}>
          <TextField
            ...
            value={activeItem?.pl}
            onChange={(e) =>
              setActiveItem({ ...activeItem, pl: e.target.value })
            }
          />
        </MDBox>
        <MDBox sx={{ width: '100%', mt: 2 }}>
          <TextField
            ...
            label="Kinh độ"
            variant="outlined"
            value={activeItem?.long}
            onChange={(e) =>
              setActiveItem({ ...activeItem, long: e.target.value })
            }
          />
        </MDBox>
        <MDBox sx={{ width: '100%', mt: 2 }}>
          <TextField
            ...
            label="Vĩ độ"
            variant="outlined"
            value={activeItem?.lat}
            onChange={(e) =>
              setActiveItem({ ...activeItem, lat: e.target.value })
            }
          />
        </MDBox>
      </DialogContent>
...
const DiemQuanTracManagement = () => {
  ...
  const [dsDiemQuanTrac, setDsDiemQuanTrac] = useState([])
  ...
  const fetchData = async () => {
    const res = await api.getDiemKhoanTrac(
      `limit=${limit}&page=${page}&search=${dataFilter.searchText}`
    )
    if (res.success) {
      setDsDiemQuanTrac(res.data.bandos)
...
export default DiemQuanTracManagement
